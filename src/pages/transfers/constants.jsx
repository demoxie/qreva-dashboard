    ...
    renderCell: (params) => (
      <span className="font-general text-sm flex items-center gap-3">
        ...
        <p className="font-medium leading-[148%] text-[#1E1E1E]">
          {params.row.location || params.row.name}
        </p>
      </span>
    ),
  },
      ...
      <button
        onClick={() =>
          handleViewRegionDetails(
            params.row._id || params.row.id || params.row.location,
          )
        }
      ...
      <div className="flex flex-col justify-center h-full">
        <div className="text-sm font-general font-medium flex items-center h-full">
          {params.row.senderName ||
            params.row.metadata?.debitAccountName ||
            params.row.transferId?.accountName ||
            "-"}
        </div>
      </div>
    ),
  ...
  },
  {
    ...
    renderCell: (params) => (
      <span className="text-sm text-gray-500 flex items-center h-full">
        {params.value
          ? new Date(params.value).toLocaleDateString("en-NG", {
              year: "numeric",
              month: "short",
              day: "numeric",
              hour: "2-digit",
...
const createTransactionActions = ({
  ...
  },
  {
    ...
    onClick: (transaction) => {
      navigate(
        `/transfers/details/transaction/${transaction._id || transaction.id}`,
      );
    },
  },
];
