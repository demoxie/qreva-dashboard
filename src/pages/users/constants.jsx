...
export const userStats = [
  { label: 'Total Users', value: '0', change: '+0%', subtext: 'in last 24 hours' },
  { label: 'Total Accounts', value: '0', change: '+0%', subtext: 'in last 24 hours' },
  { label: 'Total Agents', value: '0', change: '+0%', subtext: 'in last 24 hours' },
  { label: 'Total Aggregators', value: '0', change: '+0%', subtext: 'in last 24 hours' },
  { label: 'Total Aggregator Managers', value: '0', change: '+0%', subtext: 'in last 24 hours' }
];
...
export const userTransactionColumns = [
  {
    ...
    renderCell: (params) => {
      const name = params.row.senderName
        ...
        || params.row.title
        || '-';
      const acc = params.row.phoneNumber
      ...
      return (
        <div className='flex flex-col justify-center h-full'>
          <div className="text-sm font-general text-[#1E1E1E] font-medium">{name}</div>
  ...
  },
  {
    ...
    renderCell: (params) => {
      ...
      const statusClass = statusColors[params.value] || 'bg-gray-50 border border-gray-300 text-gray-500';
      return (
        <span className={`px-3 py-1.5 ${statusClass} text-xs rounded-md font-general tracking-wider font-semibold leading-[145%]`}>
          {params.value}
        </span>
      );
    }
  ...
  },
  {
    ...
    renderCell: (params) => {
      ...
      if (isNaN(d)) return <span className="text-sm text-[#1E1E1E] font-general flex items-center h-full">{val}</span>;
      return (
        <span className="text-sm text-[#1E1E1E] font-general flex items-center h-full">
          {d.toLocaleDateString('en-NG', { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
        </span>
      );
    }
  },
